...
altitude_change_threshold = 20  # meters
# New parameter to merge thermal segments separated by short gaps.
max_gap_seconds = 60 # seconds, maximum time gap to consider two segments part of the same thermal
...
def main():
    ...
    if not all_thermal_strengths:
        ...
        print(f"\n--- Distance Between Thermals Distribution (Distances <= {max_thermal_distance_km}km) ---")
    ...
    if filtered_thermal_distances:
        axes[1].hist(filtered_thermal_distances, bins=15, density=True, color='lightgreen', edgecolor='black', range=(0, 50))
        axes[1].set_title(f'Probability Distribution of Thermal Distance (<= {max_thermal_distance_km}km)')
...
if __name__ == "__main__":
    main()
0  # seconds, maximum time gap to consider two segments part of the same thermal
# New parameter to filter out large distances that skew the distribution.
